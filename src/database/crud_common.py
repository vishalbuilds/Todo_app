...
from database.models import  User,List
from sqlalchemy.ext.asyncio import AsyncSession


type DB_TYPE= List | User
...
class CRUD:
    ...
    async def get_by_id_pk_unqie_return(self, key: str, value: str) -> DB_TYPE:
        ...
        column = getattr(self.db_name,key)
        if column is None:
    ...
    async def update_by_id(self, id: str, key: str, value: str) -> Dict | None:
        ...
        stmt = update(self.db_name).where(self.db_name.id == id).values({column: value}).returning(self.db_name)
        result = await self.session.execute(stmt)
        await self.session.commit()
